...
class err:
    ...
    def __init__(self, ny:int, nu:int, n_lin:int, yid:np.ndarray, uid:np.ndarray, cte:bool = False) -> None:
    ...
    def update(self, ny:int, nu:int, n_lin:int, yid:np.ndarray, uid:np.ndarray, cte:bool = False) -> None:
        self.nu = nu if isinstance(nu, np.ndarray) else np.array([nu])
        self.ny = ny
        self.n_lin = n_lin
        self.y = yid
    ...
    def __termos(self) -> None:
        ...
        numRegLin = self.ny+np.sum(self.nu) # Número de regressores lineares (sem contar com o regressor constante)
        ...
        for i in range(self.n_lin):   
            ...
            for j in range(len(x_1)):
                ...
                if numRegLin>=np.array(x_1[j]).shape[0]:
                    ...
                    self.termos_matrix=np.concatenate((self.termos_matrix,x_4),axis=0)       
    ...
    def __regressores(self) -> np.array:
        ...
        numRegLin = self.ny+np.sum(self.nu) # número de regressors lineares (sem contar com o regressor constante)
        numReg = self.termos_matrix.shape[0] # número de regressores candidatos para o espaço de regressores   
        ...
        for i in range(numReg):
            for j in range(numRegLin):
                if self.termos_matrix[i,j] == 1:
                    ...
                    psi[:,i]=np.multiply(psi[:,i],np.power(reg[:,j],self.termos_matrix[i,j]))
    ...
    def __err(self):
        ...
        try:
            ...
            raise Exception("Nome 'nmax' não definido, favor executar os métodos na ordem: termos -> regressores -> err")
